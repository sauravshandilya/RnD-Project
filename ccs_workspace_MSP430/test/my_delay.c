...
void delay_30micro(unsigned int number_of_milliseconds)
{
	TA0CCTL0 |= CM_0 + CCIE;	//Enable compare mode + Interrupts on Timer
	TA0CTL |= TASSEL_1;			//Use ACLK as source for timer - 32KHz

	unsigned int i = 0;
	unsigned int delayCycles = 1; // for 31 micro sec delay Number of cycles in the timer - smallest possible delay with ACLK
